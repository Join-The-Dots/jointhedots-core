...
const MDX_Block = new TokenPattern(XHTMLTokId.MDX_Block, "MDX_Block", "```", (tok) => {
   ...
   // Parse advanced block
      ...
      const indent = input.slice(content_end + 1, block_end)
      const content = parse_content_embeds(source, format,
         block_mark.slice(0, -1), head_end + 1, block_end)
