...
function parse_paragraph(lines: (string | AST.Any)[][], withtext: boolean, rank: number, out: AST.Any[]): AST.Any[] {
   ...
   if (rank > 0) {
      if (rank === 1) { // Package previous rank 0 elements before
         if (out.length > 1) {
            out = [{
               type: "Block",
               format: AST.BlockFormat.Paragraph,
               content: out,
            }]
         }
         else {
...
function parse_jsx_document(stream: TokenStream, matchEnd?: (tok: Token) => boolean): AST.Any[] {
   let out: AST.Any[] = []
   let line_chunks: (string | AST.Any)[] = null
   let line_withtext = false
   let paragraph_lines: (string | AST.Any)[][] = null
...
function parse_jsx_attributes(stream: TokenStream): AST.ElementAttribute[] {
   let attributes: AST.ElementAttribute[] = null
   let stok: Token = null
   for (; ;) {
      stok = stream.next()
...
function parse_jsx_element(tag: string, stream: TokenStream): AST.ElementPrimitive {

   // Parse node and atttributes
   const node: AST.ElementPrimitive = {
      type: "Element",
      tag,
   ...
   if (stok.id === XHTMLTokId.MARKUP_TagClosure) {
      ...
      node.content = parse_jsx_document(doc_stream, (tok) => {
...
function parse_content_embeds(source: TokenStream, mark: string, start: number, end: number): AST.Any[] {
   const { input } = source
   const content: AST.Any[] = []
      ...
      if (embed_pos >= start && embed_pos < end) {
         try {
            ...
            if (embed_end < end) {
               ...
               content.push({ type: "Literal", start, end, value: chunk }, node)
               start = embed_end + 1
               pos = start
            }
            else {
   ...
   if (start < end) {
      ...
      content.push({ type: "Literal", start, end, value: chunk })
   }
   return content
}
...
const MDX_Block = new TokenPattern(XHTMLTokId.MDX_Block, "MDX_Block", "```", (tok) => {
   ...
   if (isInline) {
      tok.data = parse_content_embeds(source,
   ...
   // Parse advanced block
      ...
      const content: AST.BlockPrimitive = {
         type: "Block",
         format: AST.BlockFormat.Code,
         lang: format,
         content: parse_content_embeds(source, block_mark.slice(0, -1),
...
export function parseDocumentFromMDX(text: string): AST.DocumentPrimitive {
   ...
   if (match) {
      frontmatter = match[1]         // YAML block
      body = text.slice(match[0].length) // Slice the rest
   } else {
      frontmatter = null
      body = text // no frontmatter, keep whole file
   }
   ...
   const stream = new TokenStream(body.replaceAll("\r", ""), Tokenizer_Document)
   return {
      type: "Document",
      metadata: frontmatter && YAML.parse(frontmatter),
      content: parse_jsx_document(stream),
   }
}
