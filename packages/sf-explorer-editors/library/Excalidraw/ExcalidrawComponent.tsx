...
import ImageResizer from '../../designer/nodes/Images/ImageResizer'
...
export function ExcalidrawComponent({
  ...
  const node = instr.getController() as ComponentNode
  ...
  const setData = (
    ...
    return editor.update(() => {
      node.setProperty("data", JSON.stringify({
        appState: aps,
        elements: els,
        files: fls,
      }))
  ...
  return (
    <>
      ...
      {elements.length > 0 && (
        <button
          ref={buttonRef}
          className={`excalidraw-button ${instr.isSelected ? 'selected' : ''}`}>
          <ExcalidrawImage
