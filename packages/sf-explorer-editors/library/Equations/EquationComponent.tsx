...
import { ComponentNode } from '../../designer/nodes/Component/ComponentNode'
...
export function EquationComponent({
  ...
  const nodeKey = instr.getController()?.["getKey"]?.()

  const onHide = useCallback(
    (restoreSelection?: boolean) => {
      setShowEquationEditor(false)
      node.setProperty("equation", equationValue)
      if (restoreSelection) {
        node.selectNext(0, 0)
      }
    },
    [editor, equationValue, node],
  )
