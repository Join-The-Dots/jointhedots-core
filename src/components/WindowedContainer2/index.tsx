...
import { FeaturesContext, IFeatureWindowsFrame, ReactFeaturesContext, WindowInstance } from "components/FeaturesLayout"
...
export default class WindowedContainer extends React.Component implements IFeatureWindowsFrame {
   ...
   loadDisplayLayout(displayLayout): StateType["panels"] {
      ...
      for (const dockId in displayLayout) {
         ...
         const panel: PanelProps = {
            ...panelDesc,
            id: dockId,
            current: this.context.getWindowInstance(panelDesc.current),
            items: [],
         }
         panelDesc.items && panelDesc.items.forEach(wid => {
            const wnd = this.context.getWindowInstance(wid)
            if (wnd) {
               wnd.dockId = dockId
