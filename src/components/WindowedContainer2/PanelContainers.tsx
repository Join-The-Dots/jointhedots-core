...
import { WindowContainer, WindowInstance } from "components/FeaturesLayout"
...
export class SidePanelContainer extends React.Component {
   ...
   render() {
      const { current, vertical, size, children } = this.props
      const extent = (size + "%")
      ...
      if (current) {
         if (children) {
            ...
            return (<div style={style} className="WND_panel_container_vertical">
               {children}
               <WindowContainer
            ...
            const style = {
               width: vertical ? extent : "auto",
               height: vertical ? "auto" : extent,
            }
            return (<WindowContainer
               ref={this.useContainer}
               className="WND_panel_container_side WND_panel_container"
