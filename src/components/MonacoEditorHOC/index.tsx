...
import { editor as Editor, languages as Lang } from "monaco-editor"
import "./index.scss"
...
export class StandardLanguageProvider<T> implements MonacoLanguageProvider<T> {
   protected contexts = new Map<Editor.ITextModel, T>
   constructor(
      readonly language: string,
   ) {
   }
   ...
   return class MonacoEditor extends React.Component<MonacoPropsType<T>> {
      ...
      setValue(value: string) {
         if (this.model) {
            ...
            const model = Editor.createModel(value, provider.language)
